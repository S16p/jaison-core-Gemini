...
@app.route('/')
async def index():
    return "<h1>J.A.I.son Core Server is running</h1>", 200
...
async def _request_job(job_type: JobType):
    ...
    try:
        ...
        return create_response(200, f"{job_type} job created", {"job_id": job_id}, headers=cors_header)
    except Exception as err:
        logging.error(f"Error occured for {job_type} API request", stack_info=True, exc_info=True)
        return create_response(500, str(err), {}, headers=cors_header)
